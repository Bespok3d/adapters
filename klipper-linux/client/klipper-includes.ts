...
// config. Enrolment does it once and switching bespok3d back on does it again, so both go through
// here; the jinni owns the same edit on the device side, for the times there is a daemon to ask.
//
// The config path is not a fixed literal on this adapter: it was read off the printer's own service
// files, so it reaches the shell quoted and reaches the Python program as an argument, never as
// part of the program's text.
...
export function bespok3dIncludeCommand(cfgPath: string, include: Bespok3dInclude): string {
  const quotedPath = shellQuote(cfgPath)

  return `grep -q '${include.pattern}' ${quotedPath} 2>/dev/null || python3 -c "
import sys
path = sys.argv[1]
