...
It governs the jinni runtime and test INFRASTRUCTURE (conftest, fakes); the test SUITES (test_*.py,
...
_SOURCE_DIRS = ("jinni", "tests")
_EXCLUDED_DIRS = {"__pycache__", ".venv", ".pytest_cache", ".mypy_cache", ".ruff_cache"}


def _is_governed(path: Path) -> bool:
    """A .py file the ratchet measures: jinni runtime or test infra. A test SUITE (test_*.py)
