...
def test_klipper_restart_is_a_core_service_action() -> None:
    ...
    assert effect.restarts_services == (KLIPPER_SERVICE,)
...
def test_moonraker_restart_is_detected_across_init_systems() -> None:
    assert _effect(_U1LikeJinni(), "/etc/init.d/S61moonraker restart").restarts_services == (MOONRAKER_SERVICE,)  # noqa: E501
    assert _effect(_U1LikeJinni(), "systemctl reload moonraker").restarts_services == (MOONRAKER_SERVICE,)  # noqa: E501
    assert MOONRAKER_SERVICE not in _effect(_U1LikeJinni(), "/etc/init.d/S60klipper restart").restarts_services  # noqa: E501
...
def test_display_restart_interrupts_a_print_without_being_a_core_service() -> None:
    ...
    assert effect.deferrable
    assert effect.restarts_services == ()
...
def test_a_generic_box_has_no_service_actions() -> None:
    ...
    assert all(effect == CommandEffect(False, (), None) for effect in effects)
