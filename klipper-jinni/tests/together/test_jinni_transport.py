...
from jinni.klipper_vocab import KLIPPER_SERVICE, RESTART_KLIPPER
...
def test_health_call_timeout_exceeds_its_probe_budget(
    ...
    monkeypatch.setattr(jinni_client.dispatch.protocol, "call", fake_call)
    jinni_client.use_socket(socket_path)
    jinni_client.health()
...
async def test_run_actions_executes_on_the_jinni_over_the_socket(socket_path: str) -> None:
    # The daemon resolves the commands; the jinni RUNS them (ADR-0037). The seam's run_actions must
    # round-trip the ActionResult shape, so a real command's output comes back through the socket.
    server = await service.serve(socket_path, _FakeJinni())
    jinni_client.use_socket(socket_path)
    async with server:
        results = await asyncio.to_thread(jinni_client.run_actions, ["echo wired"])
    assert [(result.ok, result.output) for result in results] == [(True, "wired")]
